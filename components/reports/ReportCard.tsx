...
const ReportCard: React.FC<ReportCardProps> = ({
  ...
  return (
    <div className="bg-app-deep border border-app-line rounded-lg p-4 sm:p-6 hover:border-app-line transition-colors">
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-bold text-app-text mb-2 break-words">{item.title}</h3>
      ...
      <div className="space-y-2 text-sm text-app-text2 ml-9">
        <div className="flex items-center gap-2">
          <span className="text-app-text3">Оборудование:</span>
        ...
        {item.opo_name && (
          <div className="flex items-center gap-2">
            <span className="text-app-text3">ОПО:</span>
            <span>
              {item.opo_name}
        ...
        {(item.itemType === 'questionnaire' || item.itemType === 'report') && item.inspector_name && (
          <div className="flex items-center gap-2">
            <User size={16} className="text-app-text3" />
            <span className="text-app-text3">Инженер:</span>
            <span className="font-medium">{item.inspector_name}</span>
            {item.inspector_position && (
              <span className="text-app-text3">({item.inspector_position})</span>
            )}
          </div>
        )}
        {item.itemType === 'questionnaire' && item.inspection_date && (
          <div className="flex items-center gap-2">
            <Calendar size={16} className="text-app-text3" />
            <span className="text-app-text3">Дата обследования:</span>
            <span>{formatDate(item.inspection_date)}</span>
          </div>
        )}
        {item.created_at && (
          <div className="flex items-center gap-2">
            <span className="text-app-text3">Создан:</span>
            <span>{formatDate(item.created_at)}</span>
          </div>
        )}
        {item.file_size !== undefined && (
          <div className="flex items-center gap-2">
            <span className="text-app-text3">Размер PDF:</span>
        ...
        {item.itemType === 'questionnaire' && item.word_file_size !== undefined && (
          <div className="flex items-center gap-2">
            <span className="text-app-text3">Размер Word:</span>
        ...
        {item.itemType === 'questionnaire' && item.ndt_methods && item.ndt_methods.length > 0 && (
          <div className="mt-3 pt-3 border-t border-app-line">
            <div className="text-app-text3 text-sm mb-2">Методы неразрушающего контроля:</div>
        ...
        {item.itemType === 'questionnaire' && documentFiles && documentFiles.length > 0 && (
          <div className="mt-3 pt-3 border-t border-app-line">
            <div className="flex items-center gap-2 mb-2">
              <File size={16} className="text-app-text3" />
              <span className="text-app-text3 font-semibold">Прикрепленные документы:</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {documentFiles.map((file) => (
                <a
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-app-panel hover:bg-app-soft rounded-lg text-sm text-app-text transition-colors"
                >
                  ...
                  <span className="text-app-text3 text-xs">({formatFileSize(file.file_size)})</span>
                </a>
              ))}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {item.inspection_id && (
                <button
                  ...
                  className="px-4 py-2 bg-app-soft hover:bg-app-softer text-app-text font-semibold rounded-lg transition-colors flex items-center gap-2"
