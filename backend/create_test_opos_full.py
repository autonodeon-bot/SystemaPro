#!/usr/bin/env python3
"""
Скрипт для создания тестовых ОПО с полной информацией (survey_data)
"""
import asyncio
import sys
import os
...
async def create_test_opos_full():
    """Создание тестовых ОПО с полной информацией"""
    engine = create_async_engine(DATABASE_URL, echo=False)
    async_session = sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
    
    async with async_session() as session:
        try:
            # Получаем все активные предприятия
            enterprise_result = await session.execute(
                select(Enterprise).where(Enterprise.is_active == True)
            )
            enterprises = enterprise_result.scalars().all()
            ...
            # Создаем тестовые ОПО для каждого предприятия
            created_count = 0
            
            for enterprise in enterprises:
                print(f"\n📋 Обработка предприятия: {enterprise.name}")
                ...
                if not branch:
                    print(f"  ⚠️  Не найден филиал для предприятия {enterprise.name}, пропускаем")
                    continue
                ...
                if not workshop:
                    print(f"  ⚠️  Не найден цех для филиала {branch.name}, пропускаем")
                    continue
                
                print(f"  ✅ Найден цех: {workshop.name}")
                
                # Определяем тестовые ОПО для этого предприятия
                test_opos = [
                    {
                        "name": f"ОПО-{enterprise.name[:3].upper()}-001 - Резервуарный парк",
                        "code": f"OPO-{enterprise.name[:3].upper()}-001",
                    ...
                    {
                        "name": f"ОПО-{enterprise.name[:3].upper()}-002 - Компрессорная станция",
                        "code": f"OPO-{enterprise.name[:3].upper()}-002",
                    ...
                    {
                        "name": f"ОПО-{enterprise.name[:3].upper()}-003 - Насосная станция",
                        "code": f"OPO-{enterprise.name[:3].upper()}-003",
                    ...
                    {
                        "name": f"ОПО-{enterprise.name[:3].upper()}-004 - Установка подготовки нефти",
                        "code": f"OPO-{enterprise.name[:3].upper()}-004",
                    ...
                    {
                        "name": f"ОПО-{enterprise.name[:3].upper()}-005 - Газоперерабатывающий завод",
                        "code": f"OPO-{enterprise.name[:3].upper()}-005",
                ...
                for opo_data in test_opos:
                    ...
                    new_opo = Opo(
                        ...
                        survey_data=opo_data["survey_data"],
                        is_active=1,
                    )
                    ...
                    print(f"     Организация: {opo_data['survey_data']['organization']}")
                    print(f"     Исполнители: {opo_data['survey_data']['executors']}")
                    print(f"     Документов заполнено: {sum(opo_data['survey_data']['documents'].values())}/9")
            
            await session.commit()
            print(f"\n✅ Успешно создано {created_count} тестовых ОПО с полной информацией")
...
if __name__ == "__main__":
    asyncio.run(create_test_opos_full())
