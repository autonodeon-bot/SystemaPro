...
echo ========================================
echo   ДЕПЛОЙ СИСТЕМЫ 3.23 И МОБИЛЬНОГО ПРИЛОЖЕНИЯ 3.23
echo ========================================
echo.
...
echo [2/8] Проверка APK...
if not exist "%APK_FILE%" (
    echo   ОШИБКА: APK не найден. Соберите: cd mobile ^&^& flutter build apk --release
    pause
    exit /b 1
)
echo   APK: %APK_FILE%
...
pscp -batch -hostkey %HOSTKEY% -pw "%PASSWORD%" backend\inspection_utils.py "%SERVER%:%APP_DIR%/backend/" 2>nul
