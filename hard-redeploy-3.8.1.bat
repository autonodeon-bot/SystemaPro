...
chcp 65001 >nul
echo ========================================
echo   ПОЛНАЯ ОЧИСТКА И ПЕРЕСБОРКА 3.8.1
echo ========================================
echo.
...
echo [7/9] Пересборка контейнеров БЕЗ КЭША...
...
echo [9/9] Очистка кэша nginx и перезапуск...
...
plink -batch -ssh -pw "%PASSWORD%" "%SERVER%" "docker exec es_td_ngo_frontend sh -c 'grep -r \"3.8.1\" /usr/share/nginx/html/*.html 2>/dev/null | head -2'"
...
echo.
echo ВАЖНО: Обновите страницу через Ctrl+Shift+R (жесткая перезагрузка)
echo.
pause
