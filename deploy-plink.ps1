...
Write-Host "[2.6/7] Server version env 3.7.25+62..." -ForegroundColor Yellow
Invoke-Remote "cd $REMOTE; touch .env; grep -q '^APP_VERSION=' .env && sed -i 's/^APP_VERSION=.*/APP_VERSION=3.7.25/' .env || echo APP_VERSION=3.7.25 >> .env; grep -q '^MOBILE_APP_VERSION=' .env && sed -i 's/^MOBILE_APP_VERSION=.*/MOBILE_APP_VERSION=3.7.25/' .env || echo MOBILE_APP_VERSION=3.7.25 >> .env; grep -q '^MOBILE_APP_BUILD=' .env && sed -i 's/^MOBILE_APP_BUILD=.*/MOBILE_APP_BUILD=62/' .env || echo MOBILE_APP_BUILD=62 >> .env; grep -E '^(APP_VERSION|MOBILE_APP_VERSION|MOBILE_APP_BUILD)=' .env"
...
Write-Host "Mobile: https://neftcontrol.ru/mobile/es-td-ngo-3.7.25-62.apk" -ForegroundColor White
