...
# Add new prefs keys if missing
if ($content -notmatch "_prefsKeyOfflineEngineers") {
  $content = $content -replace "(?m)^(  static const String _prefsKeyPendingOpoSurveys .+)$", "`$1`r`n  static const String _prefsKeyOfflineEngineers = 'offline_engineers';`r`n  static const String _prefsKeyOfflineVerificationEquipment = 'offline_verification_equipment';"
}

# Extend clearOfflineCache
if ($content -notmatch "_prefsKeyOfflineEngineers") {
  # already inserted above; no-op
} else {
  $content = $content -replace "(?m)^(\s+await prefs\.remove\(_prefsKeyOfflineMode\);\s*)$", "`$1`r`n    await prefs.remove(_prefsKeyOfflineEngineers);`r`n    await prefs.remove(_prefsKeyOfflineVerificationEquipment);"
}

# Insert offline engineers/equipment methods after getOfflineAssignments
...
Write-Host "SyncService patched"
